...
monitor_features['Hz'] = 60 # this can be set to "auto" to estimate the refreshing rate of the monitor, although it can fail often
   ...
   # x = -0.5
    ...
    for i_si in range(stim['ISI2_frames']): # second period before the second beep
       ...
       # st.resp_mapping(win, st.resp_option, basic_stim,  expInfo['resp_maps'], black_resps)
        #A st.draw_contour(win,basic_stim)
        t = win.flip()
        if (i_si ==0): trial_times = np.append(trial_times, t)
    ...
    while not deci_slider.rating:
        #Ast.resp_mapping(win, st.resp_option, basic_stim,  expInfo['resp_maps'], black_resps* -1)     
        deci_slider.draw()  
